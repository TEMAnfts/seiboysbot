...
const listAllHolders = true;     // True includes duplicates
    ...
    for (let id = tokenId; id < tokenId + batchSize && id <= endTokenId; id++) {
      ...
      promises.push(
        client.queryContractSmart(contractAddress, { owner_of: { token_id: id.toString() } })
          .then(result => {
            if (result.owner && result.owner !== excludeAddress) {
              const normalizedOwner = result.owner.toLowerCase(); // Normalize address to lowercase
              ownersMap[normalizedOwner] = (ownersMap[normalizedOwner] || 0) + 1; // Increment tokens owned
            }
          })
          .catch(error => {
            console.error(`Error querying token ID ${id}:`, error.message);
